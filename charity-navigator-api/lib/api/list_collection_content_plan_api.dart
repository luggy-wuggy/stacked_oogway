...
class ListCollectionContentPlanApi {
  ListCollectionContentPlanApi([ApiClient apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<Response> getListsWithHttpInfo(String appId, String appKey, { int pageSize, int pageNum, }) async {
  ...
  /// * [int] pageNum:
  ///   Page number to return, in case the number of available objects in the result set is greater than the specified or default `pageSize`. Default value is 1. If the value of pageNum is greater than the number of pages in the result set, the method will return a 404 error. If the requested starting position, computed as `((pageNum-1) * pageSize) + 1`, is greater than the maximum supported result size of 10,000 objects, the method will return a 400 error.
  Future<List<ListObjectLink>> getLists(String appId, String appKey, { int pageSize, int pageNum, }) async {
    final response = await getListsWithHttpInfo(appId, appKey,  pageSize: pageSize, pageNum: pageNum, );
    ...
    if (response.body != null && response.statusCode != HttpStatus.noContent) {
      ...
      return (await apiClient.deserializeAsync(responseBody, 'List<ListObjectLink>') as List)
        .cast<ListObjectLink>()
        .toList(growable: false);

    }
    return Future<List<ListObjectLink>>.value();
  }
}
